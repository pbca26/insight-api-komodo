...
TokensController.prototype.decode = function(req, res) {
  var self = this;
  ...
  if (!rawtx) {
    ...
    try {
      self.node.services.bitcoind.decodeRawTransaction(rawtx, function(err, transaction) {
        if (err) {
          res.jsonp({
            msg: 'error',
...
TokensController.prototype.createtxwithnormalinputs = function(req, res) {
  var self = this;
  ...
  if (!pubkey) {
    ...
    res.jsonp({
      msg: 'error',
      result: 'Missing amount param',
    });
  } else {
    self.node.services.bitcoind.createTxWithNormalInputs(amount, pubkey, function(err, txData) {
      console.log(err)
      if (err) {
        res.jsonp({
          err: err,
...
TokensController.prototype.tokenaddccinputs = function(req, res) {
  var self = this;
  ...
  if (!pubkey) {
    ...
    res.jsonp({
      msg: 'error',
      result: 'Missing tokenid param',
    });
  } else {
    self.node.services.bitcoind.tokenAddCcInputs(tokenId, pubkey, amount, function(err, txData) {
      console.log(err)
