...
var decodeTokenDetails = function(transaction) {
  ...
  if (tokenScript.substr(0, 10) === 'OP_RETURN ' && tokenScript.length > 76) {
    ...
    tokenDetails.owner = transaction.outputs[2].scriptAsm.substr(0, 66);
    tokenDetails.ownerAddress = new bitcore.PublicKey(contractOwner, {network: bitcore.Networks.livent}).toAddress().toString();
