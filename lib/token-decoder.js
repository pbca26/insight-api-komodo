...
var decodeTokenDetails = function(transaction) {
  ...
  if (tokenScript.substr(0, 10) === 'OP_RETURN ' && tokenScript.length > 76) {
    ...
    var readVar = (hex, type) => {
      ...
      var varDecoded = oldNftPayloadNftBits.indexOf(hex.substr(0, 4)) > -1 ? Buffer.from(tokenScript.substr(4, hex.length), 'hex').toString().trim() : Buffer.from(tokenScript.substr(varHexLen, varLen * 2), 'hex').toString().trim();
      //console.log(varLen)
      //console.log('varLen', varLen);
      //console.log('varHexLen', varHexLen);
      //console.log('decoded var', type === 'value' ? varLen : varDecoded);
      var hexNext = tokenScript.substr(type === 'value' ? varHexLen : varHexLen + varLen * 2, hex.length);
